                        ...
                        <form action="{{ route('accommodation.update', $accommodation->slug) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-outline mb-4">
                                ...
                                <div class="row" id="image-preview-area">
                                    @if(is_array($accommodation->image))
                                        @foreach($accommodation->image as $imagePath)
                                            <div class="col-2 mb-2">
                                                <img src="{{ asset('storage/' . $imagePath) }}" class="img-fluid rounded" style="width: 100px;" alt="Existing Image">
                                            </div>
                                        @endforeach
                                    @endif
                                </div>
                            ...
                            <div class="form-outline mb-4">
                                ...
                                <div id="room-types-container" class="relative">
                                    ...
                                    <!-- Dropdown Menu -->
                                    <div id="room-types-dropdown" class="absolute w-full bg-white border rounded shadow-lg mt-1 p-4">
                                        @foreach($roomTypes as $roomType)
                                        <label class="flex items-center space-x-4 text-lg font-semibold">
                                            <input type="checkbox" name="room_types[]" value="{{ $roomType->id }}"
                                                {{ in_array($roomType->id, $accommodation->room_types ?? []) ? 'checked' : '' }}
                                                class="h-5 w-5">
                            ...
                            <button type="submit" class="btn btn-primary rounded-pill btn-block mb-3">Update</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('click', function(event) {
            ...
            if (selectedRoomTypes.contains(event.target)) {
                dropdown.style.maxHeight = dropdown.style.maxHeight ? '' : '300px';
        ...
        function updateSelectedRoomTypesText() {
        ...
        // Run update function on page load and checkbox change
        window.addEventListener('load', updateSelectedRoomTypesText);
        checkboxes.forEach(checkbox => checkbox.addEventListener('change', updateSelectedRoomTypesText));
    </script>
</x-app-layout>
